...
<body class="main_body" id="body">
	...
	<div class="maincontent" style="text-align:center;">
		...
		<tr>
			<td height="20" align="center" bgcolor="#FFFFFF">
				<a href="../newExhibition/list.php?catId=<?=$row->catId?>" class="liw" title="点击进入添加下一级栏目">
				<? echo $row->catName?>
				</a>
			</td>
	...
	<div class="maincontent" style="text-align:center;">
		<table width="100%" height="27"  cellpadding="0" cellspacing="0" style="background:url(../../resources/images/sechbg.gif) repeat-x">
			<tr>
				...
				<td width="45%" align="right" style="padding-right:10px;">
					<a href="../newExhibition/initAdd.php?catId=<?=$_GET["catId"]?>" title="添加资讯">
					[添加资讯]					
					</a>				
				</td>
			</tr>
		...
		$pagesize = 10;		
		$total = $newExhibitionManager->GetNewExhibitionCountByCatId($catId);
		if($total == ""){
		...
		$newExhibitions = $newExhibitionManager->GetNewExhibitionListByCatId($catId, $pagingbean->offset(), $pagesize);
		if ($newExhibitions != null) {
			foreach($newExhibitions as $row) {
		...
		<tr>
			...
			<td height="20" align="center" bgcolor="#FFFFFF">
				<a href="../newExhibition/initEdit.php?action=edit&nId=<?=$row->nId?>&catId=<?=$catId?>" class="liw" title="点击进入添加编辑页面">
				<? echo $row->title?>
				</a>
			</td>
			<td align="center" bgcolor="#FFFFFF">
			...
			<td align="center" bgcolor="#FFFFFF"><?=$row->seqNo?></td>
			<td align="center" bgcolor="#FFFFFF">
				<a href="../newExhibition/initEdit.php?action=edit&nId=<?=$row->nId?>&catId=<?=$catId?>"> 
					<img src="../../resources/images/edt.gif" width="16" height="16" border="0" />编辑
				</a>
				&nbsp;
				<a href="../newExhibition/del.php?action=del&nId=<?=$row->nId?>&catId=<?=$catId?>" onClick="return confirmDel();">
	...
	?>
	<form id="modifyForm" name="modifyForm" action="../newExhibition/modify.php" method="post">
