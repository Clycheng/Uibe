              ...
              <td width="20%" height="24" align="center">
                身份证号
				      ...
				      <td width="15%" height="24" align="center">
						...
						$records = $userManager->GetUserList($pagingbean->offset(), $pagesize);
						if ($records != null) {
							foreach($records as $record) {
							
						?>
						<tr>
						  <td align="center" bgcolor="#FFFFFF"><?=$record->userId?></td>
							<td height="20" align="center" bgcolor="#FFFFFF">
								<a href="initEdit.php?action=edit&userId=<?=$record->userId?>" class="liw" title="点击进入编辑页面">
								<? echo $record->realName ?>
								</a>
							</td>
                ...
                foreach ($dictEntryList as $dictEntry) {
                  if ($record->userType == $dictEntry->entryId) {
                    echo $dictEntry->entryName;
                  }
                }
                ?>
				      </td>
							<td align="center" bgcolor="#FFFFFF"><?=$record->number?></td>
							<td align="center" bgcolor="#FFFFFF">
								<a href="../course/list.php?userId=<?=$record->userId?>"> 
									<span class="ui-state-default ui-corner-all" style="padding:6px 16px 6px 16px; float:left">
                    考试成绩
                  </span>
								</a>
								&nbsp;
								<a href="initEdit.php?action=edit&userId=<?=$record->userId?>"> 
                    ...
                    <span class="ui-icon ui-icon-pencil" style="float:left"></span>编辑
								...
								&nbsp;
								<a href="del.php?action=del&userId=<?=$record->userId?>" onClick="return confirmDel();">
									<span class="ui-state-default ui-corner-all" style="padding:6px 12px 6px 4px; float:left">
